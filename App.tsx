...
const App: React.FC<{ username: string; onLogout: () => void }> = ({ username, onLogout }) => {
  ...
  }, [username, history, purchaseLogs, playerStats, settings, unlockedAchievements]);
  ...
  }, [history, purchaseLogs, unlockedAchievements, calculatePlayerStats, currentPlayerData, playerStats]);
  ...
  const handleRatePurchase = (purchaseId: string, rating: number) => {
    ...
    const currentRating = updatedRatings[pesticideName] || { totalScore: 0, count: 0 };
  ...
  const handleUpdateSettings = (newSettings: Partial<UserSettings>) => setSettings(prev => ({...prev, ...newSettings}));
